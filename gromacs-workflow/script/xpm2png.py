...
font_path = "../script/TIMES.TTF"  # 修改为你系统上的路径
...
def parse_show(inputfile, show, outputfile, ip):
    ...
    if xpm_type == "Continuous" and ip == "yes":
        ...
        cbar.ax.tick_params(labelsize=10)  # 控制刻度字体大小
        for label in cbar.ax.get_yticklabels():
            label.set_fontproperties(prop)
        #plt.title(xpm_title)
        plt.xlabel(xpm_xlabel, fontproperties=prop,  fontsize=14)
        plt.ylabel(xpm_ylabel, fontproperties=prop,  fontsize=14)
        # set the ticks
        if xpm_width < 100:
            x_tick = int(xpm_width/3)
        elif xpm_width >= 100 and xpm_width < 1000:
        ...
        ax.yaxis.set_major_locator(y_major_locator)
        plt.xticks([w for w in range(1,xpm_width+1, x_tick)],
                [xpm_xaxis[i] for i in range(0,len(xpm_xaxis), x_tick)], fontproperties=prop)
        plt.yticks([h for h in range(1,xpm_height+1, y_tick)],
                [xpm_yaxis[i] for i in range(0,len(xpm_yaxis), y_tick)], fontproperties=prop)
